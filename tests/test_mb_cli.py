...
def mock_query() -> Iterator[FunctionType]:
    ...
    Yields:
        Mock query
    """
    with patch("moe_musicbrainz.mb_cli.cli_query", autospec=True) as mock_query:
        yield mock_query
...
class TestCollectionCommand:
    ...
    def test_track(self, mock_query):
        ...
        with patch(
            "moe_musicbrainz.mb_cli.moe_musicbrainz.set_collection", autospec=True
        ) as mock_set:
    ...
    def test_remove(self, mock_query):
        ...
        with patch(
            "moe_musicbrainz.mb_cli.moe_musicbrainz.rm_releases_from_collection",
            autospec=True,
        ) as mock_rm:
            moe.cli.main(cli_args)
    ...
    def test_add(self, mock_query):
        ...
        with patch(
            "moe_musicbrainz.mb_cli.moe_musicbrainz.add_releases_to_collection",
            autospec=True,
        ) as mock_add:
            moe.cli.main(cli_args)
